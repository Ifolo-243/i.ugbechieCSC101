...
fn main(){
	...
	let date of birth:u32 = input.trim().parse().expect("Failed to parse input");
	...
	let phone number:u64 = input.trim().parse().expect("Failed to parse input");

	println!("Enter number of siblings");
	io::stdin().read_line(&mut input5).expect("Failed to read input");
	let number of siblings:i32 = trim().parse().expect("Failed to parse input");

	println!("Enter number of children");
	io::stdin().read_line(&mut input6).expect("Failed to read input");
	let number of children:i32 = input.trim().parse().expect("Failed to parse input");
